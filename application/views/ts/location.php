...
<body>
    ...
    <div id="wrapper">
                    ...
                    <ul class="nav metismenu" id="side-menu">
                        ...
                        <li class="active">
                            ...
                            <ul class="nav nav-second-level">
                                ...
                                <li class="active"><a href="<?php echo base_url() ?>ts/location"><i class="fa fa-building"></i> Data Lokasi</a></li>
                ...
                <div class="row">
                    ...
                    <div class="ibox ">
                        <div class="ibox-title">
                            <button class="btn btn-primary dim" type="button" data-toggle="modal" data-target="#addlocation" ><i class="fa fa-map-marker"></i> Tambah Lokasi</button>
                        </div>
                        <div class="ibox-content">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover dataTables-location" id="table-location">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Nama Lokasi</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if($l->sts_loc == 1){
                                                    echo '<button type="button" class="btn btn-primary btn-xs active" data-loc-id="'.$l->id_loc.'" title="Lokasi Aktif"><i class="fa fa-check"></i> Aktif</button>';
                                                }else{
                                                    echo '<button type="button" class="btn btn-danger btn-xs nonactive" data-loc-id="'.$l->id_loc.'" title="Lokasi Non-Aktif"><i class="fa fa-exclamation-triangle"></i> Non-Aktif</button>';
                                                }
                                                ?>
                                            </td>
                                            <td><button type="button" class="btn btn-warning btn-xs editloc" data-loc-id="<?php echo $l->id_loc ?>" title="Edit Lokasi"><i class="fa fa-edit"></i></button> | 
                                                <button type="button" class="btn btn-danger btn-xs deleteloc" data-loc-id="<?php echo $l->id_loc ?>" title="Hapus Lokasi"><i class="fa fa-trash"></i></button>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
        ...
        <!-- Modals -->
        <div class="modal inmodal" id="addlocation" tabindex="-1" role="dialog"  aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        ...
                        <small>Silakan buat lokasi baru jika organisasi anda memiliki banyak lokasi.</small>
                    </div>
                    <form role="form" id="form" method="POST" action="<?php echo base_url()?>ts/location/add">
                        <div class="modal-body">
                                <div class="form-group">
                                    <label>Nama Lokasi</label>
                                    <input type="text" name="nama_loc" placeholder="Masukkan Nama Lokasi" class="form-control" required>
                                </div>
                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                        <i class="fa fa-map-marker modal-icon"></i>
                        <h4 class="modal-title">Aktivasi Lokasi</h4>
                        <small>Anda akan merubah status lokasi</small>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        ...
                        <small>Anda akan merubah nama lokasi</small>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        ...
                        <small>Anda akan menghapus lokasi</small>
                    </div>
    ...
    <script>
			$("#table-location").on('click', '.active', function(e) {
                e.preventDefault();
                var loc_id = $(e.currentTarget).attr('data-loc-id');
                if (loc_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ts/location/ajax?type=nonactive'); ?>',
                    data: {
                        loc_id: loc_id
                    },
                    beforeSend: function() {
            ...
            $("#table-location").on('click', '.nonactive', function(e) {
                e.preventDefault();
                var loc_id = $(e.currentTarget).attr('data-loc-id');
                if (loc_id === '') return;
                $.ajax({
            ...
            $("#table-location").on('click', '.editloc', function(e) {
                e.preventDefault();
                var loc_id = $(e.currentTarget).attr('data-loc-id');
                if (loc_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ts/location/ajax?type=editloc'); ?>',
                    data: {
                        loc_id: loc_id
                    },
                    beforeSend: function() {
            ...
            $("#table-location").on('click', '.deleteloc', function(e) {
                e.preventDefault();
                var loc_id = $(e.currentTarget).attr('data-loc-id');
                if (loc_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ts/location/ajax?type=deleteloc'); ?>',
                    data: {
                        loc_id: loc_id
                    },
                    beforeSend: function() {
