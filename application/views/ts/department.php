...
<body>
    ...
    <div id="wrapper">
                    ...
                    <ul class="nav metismenu" id="side-menu">
                        ...
                        <li class="active">
                            ...
                            <ul class="nav nav-second-level">
                                ...
                                <li class="active"><a href="<?php echo base_url() ?>ts/department"><i class="fa fa-bank"></i> Data Departemen</a></li>
                ...
                <div class="row">
                    ...
                    <div class="ibox ">
                        <div class="ibox-title">
                            <button class="btn btn-primary dim" type="button" data-toggle="modal" data-target="#adddept" ><i class="fa fa-bank"></i> Tambah Department</button>
                        </div>
                        <div class="ibox-content">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover dataTables-dept" id="table-dept">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if($l->sts_dept == 1){
                                                    echo '<button type="button" class="btn btn-primary btn-xs active" data-dept-id="'.$l->id_dept.'" title="Department Aktif"><i class="fa fa-check"></i> Aktif</button>';
                                                }else{
                                                    echo '<button type="button" class="btn btn-danger btn-xs nonactive" data-dept-id="'.$l->id_dept.'" title="Department Non-Aktif"><i class="fa fa-exclamation-triangle"></i> Non-Aktif</button>';
                                                }
                                                ?>
                                            </td>
                                            <td><button type="button" class="btn btn-warning btn-xs editdept" data-dept-id="<?php echo $l->id_dept ?>" title="Edit Department"><i class="fa fa-edit"></i></button> | 
                                                <button type="button" class="btn btn-danger btn-xs deletedept" data-dept-id="<?php echo $l->id_dept ?>" title="Hapus Department"><i class="fa fa-trash"></i></button>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
        ...
        <!-- Modals -->
        <div class="modal inmodal" id="adddept" tabindex="-1" role="dialog"  aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                        <i class="fa fa-bank modal-icon"></i>
                        <h4 class="modal-title">Tambah Department Baru</h4>
                        <small>Silakan buat department baru jika organisasi anda memiliki banyak department.</small>
                    </div>
                    <form role="form" id="form" method="POST" action="<?php echo base_url()?>ts/department/add">
                        <div class="modal-body">
                                <div class="form-group">
                                    <label>Nama Department</label>
                                    <input type="text" name="nama_dept" placeholder="Masukkan Nama Department" class="form-control" required>
                                </div>
                        </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                        <i class="fa fa-bank modal-icon"></i>
                        <h4 class="modal-title">Aktivasi Department</h4>
                        <small>Anda akan merubah status department</small>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        ...
                        <small>Anda akan merubah nama department</small>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content animated fadeIn">
                    <div class="modal-header">
                        ...
                        <small>Anda akan menghapus Department</small>
                    </div>
    ...
    <script>
			$("#table-dept").on('click', '.active', function(e) {
                e.preventDefault();
                var dept_id = $(e.currentTarget).attr('data-dept-id');
                if (dept_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ts/department/ajax?type=nonactive'); ?>',
                    data: {
                        dept_id: dept_id
                    },
                    beforeSend: function() {
            ...
            $("#table-dept").on('click', '.nonactive', function(e) {
                e.preventDefault();
                var dept_id = $(e.currentTarget).attr('data-dept-id');
                if (dept_id === '') return;
                $.ajax({
            ...
            $("#table-dept").on('click', '.editdept', function(e) {
                e.preventDefault();
                var dept_id = $(e.currentTarget).attr('data-dept-id');
                if (dept_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ts/department/ajax?type=editdept'); ?>',
                    data: {
                        dept_id: dept_id
                    },
                    beforeSend: function() {
            ...
            $("#table-dept").on('click', '.deletedept', function(e) {
                e.preventDefault();
                var dept_id = $(e.currentTarget).attr('data-dept-id');
                if (dept_id === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ts/department/ajax?type=deletedept'); ?>',
                    data: {
                        dept_id: dept_id
                    },
                    beforeSend: function() {
