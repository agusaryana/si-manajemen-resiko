...
class Report extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['ticket'] = $this->m_data->all_data_report()->result();
	...
	public function cetakLaporanPDF()
	{
		...
		$html = $this->load->view('ts/report_ticket', $data, true);
		$dompdf->load_html($html);
		$dompdf->set_paper('A4', 'Landscape');
		$dompdf->render();
		$pdf = $dompdf->output();
		$dompdf->stream('report_led', array("Attachment" => false));
	}
	...
	public function filter()
	{
		...
		$data['title'] = "Report LED";
		$data['subtitle'] = "Periode Tanggal : " . $mulai . ' Sampai Tanggal : ' . $akhir;

		if ($status == null and $id_div == null) {
			$where = array(
				'tgl_awal' => $mulai,
				'tgl_akhir' => $akhir,
			);
			$data['datafilter'] = $this->m_data->filterbytanggal($where);
		} else {
			if ($status == null) {
				$where = array(
					'tgl_awal' => $mulai,
		...
		$this->load->view('ts/print_laporan', $data);
	}
