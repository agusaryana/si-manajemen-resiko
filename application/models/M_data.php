...
class M_data extends CI_Model
{
	...
	{
		$this->db->where('sts_cat', '1');
		$query = $this->db->get_where('tb_category');
		return $query->result();
	}
	...
	public function data_ticketselesai()
	{
		...
		if ($akses == 1 || $akses == 4) {
			$this->db->select('tb_ticket.id_ticket, tb_ticket.no_ticket, tb_ticket.nama_ticket, tb_ticket.create_ticket, tb_ticket.priority_ticket, tb_ticket.approval_level, tb_ticket.exe_time,
	...
	public function tb_ticket($postData = null, $div)
	{
		if ($div == 1) {
			...
			foreach ($records as $record) {
				...
				if ($record->approval_level == '0') {
					$approval_sts = '<span type="button" class="btn btn-danger btn-xs"><i class="fa fa-close"></i> Belum Disetujui</span>';
				} elseif ($record->approval_level == '1') {
					$approval_sts = '<span type="button" class="btn btn-primary btn-xs"><i class="fa fa-check"></i> Disetujui</span>';
				}

				if ($record->sts_ticket == '1') {
					$sts = '<button type="button" class="btn btn-danger btn-xs" title="Open">Open</button>   | <button type="button" class="btn btn-danger btn-xs delete" data-ticket-id="' . $record->id_ticket . '" title="Hapus Request"><i class="fa fa-trash"></i></button>';
				} elseif ($record->sts_ticket == '2') {
					$sts = '<button type="button" class="btn btn-warning btn-xs timeline" data-ticket-id="' . $record->no_ticket . '" title="Proses">Proses</button> | <button type="button" class="btn btn-danger btn-xs delete" data-ticket-id="' . $record->id_ticket . '" title="Hapus Request"><i class="fa fa-trash"></i></button>';
				} elseif ($record->sts_ticket == '3') {
					$sts = '<span type="button" class="btn btn-primary btn-xs timeline" data-ticket-id="' . $record->no_ticket . '" title="Selesai"><i class="fa fa-check"></i> Selesai</span>';
				}
				...
				$data[] = array(
					...
					"status_approval" 	=> '<button type="button" class="btn btn-xs approve" data-ticket-id="' . $record->id_ticket . '" title="Approval">' . $approval_sts . '</button>',
					"status_selesai"	=> $sts,
					"status"			=> $approval_sts,
				);
			}
			...
			## Fetch records
			$this->db->select('tb_ticket.id_ticket, tb_ticket.id_div, tb_ticket.no_ticket, tb_ticket.create_ticket, tb_ticket.priority_ticket, tb_ticket.nama_ticket, tb_ticket.exe_time, tb_ticket.approval_level
			...
			foreach ($records as $record) {
				...
				// 	$sts = 'On Progres';
				// } elseif ($record->sts_ticket == '3') {
				// 	$sts = '<span type="button" class="btn btn-primary btn-xs"><i class="fa fa-check></i> Closed</span>';
				// }
	...
	public function get_filtered_data($tgl_awal, $tgl_akhir, $status, $id_div, $id_cat)
	{
		$this->db->select(
			'
        tb_ticket.id_ticket, 
		...
		$this->db->join('tb_category', 'tb_category.id_cat = tb_ticket.id_cat');
		$this->db->join('tb_user', 'tb_user.id_user = tb_ticket.id_user', 'left');
		// Tambahkan filter sesuai dengan inputan dari form
		if ($tgl_awal != 0) {
			$this->db->where('tb_ticket.create_ticket >=', $tgl_awal);
		}
		if ($tgl_akhir != 0) {
			$this->db->where('tb_ticket.create_ticket <=', $tgl_akhir);
		}
		if ($status != 0) {
			$this->db->where('tb_ticket.sts_ticket', $status);
		}
		if ($id_div != 0) {
			$this->db->where('tb_ticket.id_div', $id_div);
		}
		if ($id_cat != 0) {
			$this->db->where('tb_ticket.id_cat', $id_cat);
		}
